...
interface SwingFeedbackCommentDao {
    ...
    @Query("SELECT * FROM swing_feedback_comment WHERE userID = :userId AND videoName = :videoName")
    fun getSwingFeedbackComment(userId: Long, videoName: String): SwingFeedbackComment

    @Query("DELETE FROM swing_feedback_comment WHERE userID = :userId AND videoName = :videoName")
