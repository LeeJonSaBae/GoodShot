...
class ResultCall<T>(private val delegate: Call<T>, private val retrofit: Retrofit) : Call<Result<T>> {
                    ...
                    // 응답은 왔지만 400번 등 에러가 발생했을 때
                        ...
                        if(response.errorBody() == null){
                            callback.onResponse(this@ResultCall,
                                Response.success(Result.failure(RetrofitException("에러 메세지가 없습니다", 404, HttpException(response)))))
                        }else{
                            ...
                            callback.onResponse(this@ResultCall,
                                Response.success(Result.failure(RetrofitException(message, code, HttpException(response))))
                            )
                        }
                    }
                }
